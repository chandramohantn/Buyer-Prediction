...
m = merge([m13, m23, m33], mode='concat')
g6 = Dense(2, activation='softmax')(m)
model = Model(input=[input1, input2, input3], output=[g6])
...
for e in range(20):
	...
	for b in range(ls):
		...
		#l = model.train_on_batch([x1, x2, x3], y)
		l = model.train_on_batch([x1, x2, x3], y, class_weight=class_weight)
		train_loss += l
	print('Epoch: ' + str(e) + ' Loss: ' + str(train_loss))
	model.save_weights('model/lstm_stacking_buy_w_' + str(e), overwrite=True)
	v_score = model.evaluate([X_val1, X_val2, X_val3], y_val)
	t_auc.append(np.log(train_loss * 1.0 / ls))
	v_auc.append(np.log(v_score))
	if score > v_score:
		score = v_score
		model.save_weights('model/best_lstm_stacking_buy_w', overwrite=True)
...
plt.ylabel('Log loss')
plt.xlabel('Epochs')
plt.savefig('LSTM_stacking_train_val_loss.png')
plt.close()

model.load_weights('model/best_lstm_stacking_buy_w')
