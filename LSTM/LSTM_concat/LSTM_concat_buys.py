...
from keras.layers import LSTM, Dense, Activation, TimeDistributedDense
import numpy as np
...
X_train = Process_data(X_train, max_len, 64, item_vec)
X_train = np.concatenate((X_train1, X_train), axis=2)
[X_val, y_val] = load_data(val_path, 1)
X_val = Process_data(X_val, max_len, 64, item_vec)
X_val = np.concatenate((X_val1, X_val), axis=2)
...
[X_val1, y_val] = load_data(val_path, 1)
X_val = np.concatenate((X_val, X_val1), axis=2)
'''
t_auc = []
v_auc = []
...
for e in range(20):
	...
	model.save_weights('model/lstm_concat_buy_w_' + str(e), overwrite=True)
	v_score = model.evaluate(X_val, y_val)
	t_auc.append(np.log(train_loss * 1.0 / ls))
	v_auc.append(np.log(v_score))
	if score > v_score:
		score = v_score
		model.save_weights('model/best_lstm_concat_buy_w', overwrite=True)
...
plt.ylabel('Log loss')
plt.xlabel('Epochs')
plt.savefig('LSTM_concat_train_val_loss.png')
plt.close()
