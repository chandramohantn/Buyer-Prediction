...
for e in range(50):
	...
	model.save_weights('model/lstm_moe_end2end_buy_w_' + str(e), overwrite=True)
	v_score = model.evaluate([X_val1, X_val2, X_val3, X_val], y_val)
	t_auc.append(np.log(train_loss * 1.0 / ls))
	v_auc.append(np.log(v_score))
	if score > v_score:
		score = v_score
		model.save_weights('model/best_lstm_moe_end2end_buy_w', overwrite=True)
...
plt.ylabel('Log loss')
plt.xlabel('Epochs')
plt.savefig('LSTM_moe_end2end_train_val_loss.png')
plt.close()
