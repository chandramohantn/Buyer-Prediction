...
for e in range(20):
	...
	model.save_weights('model/sequence_buy_w_' + str(e), overwrite=True)
	if score > v_score:
		score = v_score
		model.save_weights('model/best_sequence_buy_w', overwrite=True)
...
plt.ylabel('Log loss')
plt.xlabel('Epochs')
plt.savefig('Sequence_train_val_loss.png')
plt.close()

model.load_weights('model/best_sequence_buy_w')
