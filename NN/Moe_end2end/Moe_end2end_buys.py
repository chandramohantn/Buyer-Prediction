...
print('Build model 1...')
model_1 = Sequential()
model_1.add(Dense(64, input_dim=n, init='uniform', activation='sigmoid'))
...
print('Build model 2...')
model_2 = Sequential()
model_2.add(Dense(64, input_dim=n, init='uniform', activation='sigmoid'))
...
print('Build model 3...')
model_3 = Sequential()
model_3.add(Dense(64, input_dim=n, init='uniform', activation='sigmoid'))
...
for e in range(50):
	...
	model.save_weights('model/moe_end2end_buy_w_' + str(e), overwrite=True)
	v_score = model.evaluate([X_val1, X_val2, X_val3, tmp2], y_val)
	t_auc.append(np.log(train_loss * 1.0 / ls))
	v_auc.append(np.log(v_score))
	if score > v_score:
		score = v_score
		model.save_weights('model/best_moe_end2end_buy_w', overwrite=True)
...
plt.ylabel('Log loss')
plt.xlabel('Epochs')
plt.savefig('Moe_end2end_train_val_loss.png')
plt.close()

model.load_weights('model/best_moe_end2end_buy_w')
