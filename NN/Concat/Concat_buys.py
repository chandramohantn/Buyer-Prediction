...
score = 10000000000
batch_size = 512
[X_train1, y_train] = load_data('../../../Train_data/Train/', 'Session/Session.dat')
[X_train, y_train] = load_data('../../../Train_data/Train/', 'Sequence/Sequence4.dat')
X_train = np.concatenate((X_train1, X_train), axis=1)
[X_train1, y_train] = load_data('../../../Train_data/Train/', 'Temporal/Temporal.dat')
X_train = np.concatenate((X_train, X_train1), axis=1)
[X_val1, y_val] = load_data('../../../Train_data/Val/', 'Session/Session.dat')
[X_val, y_val] = load_data('../../../Train_data/Val/', 'Sequence/Sequence4.dat')
X_val = np.concatenate((X_val1, X_val), axis=1)
[X_val1, y_val] = load_data('../../../Train_data/Val/', 'Temporal/Temporal.dat')
X_val = np.concatenate((X_val, X_val1), axis=1)
t_auc = []
v_auc = []
...
for e in range(20):
	...
	model.save_weights('model/concat_buy_w_' + str(e), overwrite=True)
	if score > v_score:
		score = v_score
		model.save_weights('model/best_concat_buy_w', overwrite=True)
...
plt.ylabel('Log loss')
plt.xlabel('Epochs')
plt.savefig('Concat_train_val_loss.png')
plt.close()

model.load_weights('model/best_concat_buy_w')
...
model.load_weights('model/best_concat_buy_w')
print("Testing...")
[X_train1, y_train] = load_data('../../../Train_data/Test/', 'Session/Session.dat')
[X_train, y_train] = load_data('../../../Train_data/Test/', 'Sequence/Sequence4.dat')
X_train = np.concatenate((X_train1, X_train), axis=1)
[X_train1, y_train] = load_data('../../../Train_data/Test/', 'Temporal/Temporal.dat')
X_train = np.concatenate((X_train, X_train1), axis=1)
