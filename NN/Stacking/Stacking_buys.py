...
model = Sequential()
model.add(Merge([model_1, model_2, model_3], mode='concat'))
...
for e in range(20):
	...
	model.save_weights('model/stacking_buy_w_' + str(e), overwrite=True)
	v_score = model.evaluate([X_val1, X_val2, X_val3], y_val)
	t_auc.append(np.log(train_loss * 1.0 / ls))
	v_auc.append(np.log(v_score))
	if score > v_score:
		score = v_score
		model.save_weights('model/best_stacking_buy_w', overwrite=True)
...
plt.ylabel('Log loss')
plt.xlabel('Epochs')
plt.savefig('Stacking_train_val_loss.png')
plt.close()

model.load_weights('model/best_stacking_buy_w')
...
prba = model.predict_proba([X_train1, X_train2, X_train3])
